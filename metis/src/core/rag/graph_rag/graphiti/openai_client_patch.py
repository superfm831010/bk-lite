...
# 导入项目现有的结构化输出解析器
from src.core.llm.node.structured_output_parser import StructuredOutputParser, PromptBuilder
...
async def patched_create_structured_completion(
    ...
    parser = StructuredOutputParser(llm, max_retries=2)

    # 创建简单的prompt包装器
    simple_prompt = SimplePrompt(langchain_messages)
    ...
    try:
        ...
        result = await parser.parse_with_structured_output(
            ...
            messages={"messages": langchain_messages},
