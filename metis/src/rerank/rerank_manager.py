...
class ReRankManager:
    ...
    def rerank_documents_with_config(config: ReRankConfig, search_result: List[Document]) -> List[Document]:
        ...
        if config.model_base_url.startswith('local:'):
            ...
            reranked_docs = ReRankManager._handle_local_rerank(config, search_result)
        else:
    ...
    def _handle_local_rerank(
        ...
        try:
            # 解析协议信息
            model_type = config.model_base_url.split(':')[1]
            
            # 提取文档内容
            passages = [doc.page_content for doc in search_result]
            
            # 获取重排序实例并执行重排序
            rerank_instance = ReRankManager.get_rerank_instance(config.model_base_url)
            
            if model_type == 'bce':
                local_rerank_result = rerank_instance.rerank(config.query, passages)
            else:
                logger.warning(f"不支持的本地重排序模型类型: {model_type}")
                return search_result
            
            # 检查重排序结果格式
            if ('rerank_ids' not in local_rerank_result or 
                'rerank_scores' not in local_rerank_result or
                len(local_rerank_result['rerank_ids']) != len(local_rerank_result['rerank_scores'])):
                logger.warning("ReRank结果格式错误或不匹配，跳过重排序")
                return search_result
    ...
    def _handle_remote_rerank(
        ...
        try:
            response = requests.post(
                config.model_base_url, headers=headers, json=data, timeout=10
            )
