...
import { TableDataItem, Organization, UserItem } from '@/app/alarm/types';
import { useTranslation } from '@/utils/i18n';
import { useLocalizedTime } from '@/hooks/useLocalizedTime';
import { ObjectItem } from '@/app/alarm/types/monitor';
import { findUnitNameById, showGroupName } from '@/app/alarm/utils/common';
...
const Information: React.FC<TableDataItem> = ({
  ...
  const { patchMonitorAlert } = useAlarmApi();
  ...
  return (
    <div className={informationStyle.information}>
      <Descriptions title={t('monitor.events.information')} column={2} bordered>
        ...
        <Descriptions.Item label={t('monitor.events.source')}>
          {objects.find(
            (item: ObjectItem) => item.id === formData.policy?.monitor_object
          )?.display_name || '--'}
        </Descriptions.Item>
        <Descriptions.Item label={t('events.source')}>
