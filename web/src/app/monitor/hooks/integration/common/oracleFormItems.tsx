    ...
    getCommonFormItems: (mode?: 'manual' | 'auto' | 'edit') => {
      return (
        <>
          ...
          <Form.Item label={t('monitor.integrations.seviceName')} required>
            <Form.Item
              noStyle
              name={`${mode === 'auto' ? 'ENV_' : ''}SERVICE_NAME`}
              rules={[
                {
                  required: true,
                  message: t('common.required'),
            ...
            <span className="text-[12px] text-[var(--color-text-3)]">
              {t('monitor.integrations.seviceNameDes')}
            </span>
          </Form.Item>
        </>
      );
