...
import { useRabbitMQFormItems } from '../../common/rabbitMQFormItems';
import { cloneDeep } from 'lodash';
import { replaceTemplate } from '@/app/monitor/utils/intergration';

export const useRabbitMQTelegraf = () => {
  const { t } = useTranslation();
  const rabbitMQFormItems = useRabbitMQFormItems();
  const pluginConfig = {
    ...
    getPluginCfg: (extra: {
      ...
      const formItems = (
        <>
          ...
          {rabbitMQFormItems.getCommonFormItems(disabledForm)}
        </>
      );
