...
import { useDmExporterFormItems } from '../../common/dmExporterFormItems';
import { cloneDeep } from 'lodash';

export const useDmExporter = () => {
  const { t } = useTranslation();
  const dmExporterFormItems = useDmExporterFormItems();
  const pluginConfig = {
    collect_type: 'exporter',
    config_type: ['dm'],
    collector: 'DM-Exporter',
    instance_type: 'dm',
    object_name: 'DM',
  };
  ...
  return {
    getPluginCfg: (extra: {
      ...
      const formItems = (
        <>
          {dmExporterFormItems.getCommonFormItems()}
      ...
      const config = {
        auto: {
          formItems: dmExporterFormItems.getCommonFormItems('auto'),
          initTableItems: {
