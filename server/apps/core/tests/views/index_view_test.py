...
def test_login_success(request_factory):
    ...
    request = request_factory.post('/login', data=json.dumps(login_data), 
...
def test_login_invalid_json(request_factory):
    ...
    request = request_factory.post('/login', data="invalid json", 
                                 content_type='application/json')
...
def test_reset_pwd_success(request_factory):
    ...
    request = request_factory.post('/reset_pwd', data=json.dumps(reset_data), 
                                 content_type='application/json')
...
def test_login_info_success(request_factory, mock_user):
    ...
    with patch('apps.core.views.index_view._create_system_mgmt_client') as mock_create_client:
        mock_client = Mock()
        mock_create_client.search_users.return_value = {
            "data": {"users": [{"id": "user123", "username": "test_user"}]}
        }
        mock_create_client.return_value = mock_client
