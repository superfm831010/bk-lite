...
class QAPairsViewSet(MaintainerViewSet, GenericViewSetFun):
        ...
        for i in params.get("document_list", []):
            chunk_data.extend(
                client.get_qa_content(
                    i.get("document_id", 0), f"knowledge_base_{params['knowledge_base_id']}", page_size=10
                )
            )
    ...
    def download_import_template(self, request):
        ...
        if file_type == "json":
            ...
            response = HttpResponse(json.dumps(tmp_data, ensure_ascii=False, indent=2), content_type="application/json")
