...
class ModelTypeViewSet(viewsets.ModelViewSet, GenericViewSetFun):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if provider_type in provider_model_map:
            ...
            model_type_counts = (
                model_queryset.values("model_type_id")
                .annotate(count=models.Count("id"))
                .values_list("model_type_id", "count")
            )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if instance.is_build_in:
            return JsonResponse({"result": False, "message": _("Built-in model types cannot be modified")})
        return super().update(request, *args, **kwargs)

    def destroy(self, request, *args, **kwargs):
        instance = self.get_object()
        if instance.is_build_in:
            return JsonResponse({"result": False, "message": _("Built-in model types cannot be deleted")})
